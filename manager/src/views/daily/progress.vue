  ...
  <a-card :bordered='false'>
          ...
          <template v-if='toggleSearchStatus'>
            ...
            <a-col :md='6' :sm='12'>
              <a-form-item label='当前环节'>
                <a-select v-model='queryParam.currentNode'>
                  <a-select-option value='1'>未受理</a-select-option>
                  <a-select-option value='2'>受理</a-select-option>
                  <a-select-option value='3'>初审</a-select-option>
                  <a-select-option value='4'>审核</a-select-option>
    ...
    <div>
      ...
      <a-table
        ...
        :scroll='{ x: 1500, y: 550 }'
      >
        <template #currentNode='{text}'>
          <span class='current-node td-span'>{{text}}</span>
        </template>
        <template #flowStatus='{text}'>
          <span class='flow-status td-span'>{{text}}</span>
        </template>
        <template #action='{record}'>
          <a-button type='link' size='small' @click='showProgress(record)'>审批进度</a-button>
        </template>
      </a-table>
    </div>
  </a-card>
  <a-modal v-model:visible='progressShow' width='700px' :footer='null' @cancel='progressShow=false'>
    <flow-show></flow-show>
  </a-modal>
</template>
    ...
    const datap = reactive({
      ...
      columns: [
        ...
        },
        {
          ...
          slots: { customRender: 'currentNode' }
        },
        {
          title: '流程状态',
          align: 'center',
